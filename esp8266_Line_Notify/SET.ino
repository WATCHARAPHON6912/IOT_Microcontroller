...
void Connect() {
  status = 1;
  for (int i = 3; i <= EEPROM.read(0) + EEPROM.read(1) + EEPROM.read(2) + 3; i++) {
...
void setwifi() {
  ...
  server.on("/get", HTTP_GET, [] (AsyncWebServerRequest * request) {
    ...
    String dd = INPUTTOKEN +  INPUTSSID +  INPUTPASS;
    Serial.println(dd.length());
    for (int i = 3; i <= dd.length() + 3; i++) {
      EEPROM.write(i, char(dd[i - 3]));
    }
    EEPROM.commit();
